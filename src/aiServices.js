...
export var _GROQ_KEY_ALIAS = []; // unused — kept to avoid reference errors in any cached code
...
export const tutorUsageKey=function(u){return "gcse:tu:"+(u||"").replace(/\W/g,"-")+":"+new
Date().toISOString().slice(0,10);};
export async function getTutorUsage(u){
try{var r=await window.storage.get(tutorUsageKey(u),true);return
r&&r.value?JSON.parse(r.value):{};} catch(e){return {};}
}
export async function incTutorUsage(u,modelName){
try{var k2=tutorUsageKey(u);var cur=await
getTutorUsage(u);cur[modelName]=(cur[modelName]||0)+1;await
window.storage.set(k2,JSON.stringify(cur),true);}catch(e){}
}
export async function pickTutorModel(u){ return TUTOR_MODELS[0]; }
...
var f=m._d.files[fi];
if(f.isText && f.textContent) fileParts.push("[Uploaded text file:
"+f.name+"]\n"+f.textContent);
else if(f.isPdf) fileParts.push("[Uploaded PDF: "+f.name+" — please analyse the content
...
if(!txt.trim() && !(m._d && m._d.files && m._d.files.some(function(f){return f.isImage;})))
continue;
...
if(typeof msgContent==="string" && msgs.length>0 && msgs[msgs.length-1].role===role &&
...
?
["meta-llama/llama-4-scout-17b-16e-instruct","llama-3.2-11b-vision-preview","llama-3.2-90b-visio
n-preview"].concat(_GROQ_MODELS)
: _GROQ_MODELS;
...
body:JSON.stringify({model:modelList[mi], messages:msgs, max_tokens:tokLimit,
temperature:0.7})
});
...
var text =
dat.choices&&dat.choices[0]&&dat.choices[0].message&&dat.choices[0].message.content;
...
export var callGeminiChat = function(_ignored, systemPrompt, messages){ return
...
// ── Shared utilities
export function _aiClamp(v, lo, hi) { return Math.max(lo, Math.min(hi, Number(v) || 0)); }
export function _aiArr(v) { return Array.isArray(v) ? v : []; }
export function _aiStr(v, fb) { return (typeof v === 'string' && v.trim()) ? v.trim() : (fb || ''); }
...
export async function buildAIPersonalisedSession(ctx, allSections, stats, fcHist, fallbackPlan) {
...
feedback: hasAns
? 'AI marking is temporarily unavailable. Self-mark using the mark scheme — check each
bullet point against your answer.'
...
var band = _aiStr(raw.band, score >= maxM * 0.8 ? 'Exceeding' : score >= maxM * 0.5 ?
...
var msLines = _aiArr(raw.missedPoints).map(function(p) { return _aiStr(p, '');
...
strengths: _aiArr(raw.strengths).map(function(p) { return _aiStr(p, '');
...
errorType: raw.errorType || (typeof detectErrorType === 'function' ? detectErrorType(q.text || '', '', q.markScheme || '', msLines) : null),
...
var ms = _aiStr(q.markScheme || q.sampleAnswer, 'Award marks for accurate, relevant
content.');
var schemaLines = [
...
'"errorType": "Knowledge Gap"|"Application Error"|"Command Word Error"|"Communication
Error"|null',
'"annotatedAnswer": [{"text":"sentence from model
answer","type":"point"|"evidence"|"explanation"|"application"}]',
'"comparisonTable": [{"student":"excerpt from student answer","expectation":"mark point they
missed"}]',
'"structureDiagram": ' + (isMath ? '["Formula","Substitution","Working","Answer+Units"]' :
'["Point","Evidence","Explanation","Application"]'),
'"workedSolution": "' + (isMath ? 'full step-by-step working with units' : 'complete model
answer') + '"',
];
var rulesBlock = [
...
isMath ? '- Award method marks (M1) for correct method even if arithmetic is wrong' : '- For
extended: use level descriptors if present in mark scheme',
isExtd ? '- Identify the HIGHEST level fully met — partial level = lower band' : '- Short answer:
1 mark per distinct accurate point matching the mark scheme',
...
var prompt = 'You are an experienced ' + board + ' GCSE examiner. Mark this answer with
precision.\n\n' +
'QUESTION: ' + (q.text || '') + '\n' +
'MAX MARKS: ' + maxM + '\n' +
'MARK SCHEME:\n' + ms + '\n' +
...
partial: [],
feedback: 'AI is temporarily offline. Compare your blurt directly against your notes: tick
correct points, circle gaps, highlight partial recalls. This comparison is itself a powerful retrieval
exercise.',
misconceptions: [],
};
}
...
remembered: _aiArr(raw.remembered).map(function(s) { return _aiStr(s, '');
}).filter(Boolean).slice(0, 12),
missed: _aiArr(raw.missed).map(function(s) { return _aiStr(s, ''); }).filter(Boolean).slice(0,
12),
...
misconceptions: _aiArr(raw.misconceptions).map(function(s) { return _aiStr(s, '');
...
'- "score": 0–100, % of key note concepts demonstrated (realistic — most students score
30–65%)\n' +
'- "feedback": 2-sentence warm encouragement + single most important gap to address
next\n\n' +
...
marks: marks,
markScheme: _aiStr(q.markScheme || q.mark_scheme, 'Award marks for accurate, relevant
content.'),
...
export async function aiServiceQuestionGenerator(subjName, board, notes, needed, markDist) {
var needPart = (needed || []).map(function(n) {
return '- ' + n.count + ' × ' + n.type + ' question(s), ' + n.marks + ' mark(s) each';
}).join('\n');
var prompt = 'You are an expert ' + (board || 'GCSE') + ' ' + (subjName || 'subject') + '
examiner.\n' +
'Generate exam-quality questions:\n' + needPart + '\n\n' +
(markDist ? 'Mark distribution: ' + markDist + '\n\n' : '') +
'Content context:\n' + ((notes || '').slice(0, 3000) || ('Standard ' + (board || 'GCSE') + ' ' +
(subjName || 'subject') + ' content')) + '\n\n' +
'RULES:\n' +
'- Use authentic ' + (board || 'GCSE') + ' command words (state, describe, explain, evaluate,
calculate, compare)\n' +
...
return {
summary: reflections.understood ? 'You understood: ' + (reflections.understood ||
'').slice(0, 120) : 'Session complete.',
keyGap: reflections.unclear ? (reflections.unclear || '').slice(0, 200) : 'Compare your
blurt against your notes to find any remaining gaps.',
nextAction: reflections.improve ? (reflections.improve || '').slice(0, 200) : 'Try a blurting
exercise on the weakest topic next time.',
...
var prompt = 'You are a warm, evidence-informed GCSE study coach. A student just
completed a revision session on ' +
...
'"nextAction":"specific evidence-based technique to use next session (e.g. spaced retrieval,
blurting, practice questions)",' +
...
export function buildTutorSystemPrompt(ctx, mode, board, subjName, topicLabel, socraticLevel) {
var lvl = typeof socraticLevel === 'number' ? socraticLevel : 2;
var imgInstr = 'When a diagram, chart or visual would GENUINELY help comprehension,
include [IMG: descriptive search term]. Educational visuals only — not decorative.';
var pedagogyBlock;
if (mode === 'homework') {
pedagogyBlock =
...
'- After each student response, celebrate correct steps: "Exactly — now what comes
next?"\n' +
...
'- For the FIRST message on any topic: ask "What do you already know about [topic]?"
before explaining\n' +
'- Use guiding questions: "What does the command word tell you?", "Which formula
applies?", "What would happen if…?"\n' +
...
? 'STUDENT\'S REVISION CONTENT (primary knowledge source — prioritise this over
general knowledge):\n' +
(ctx.notes ? '=== NOTES ===\n' + ctx.notes.slice(0, 1800) + '\n\n' : '') +
(ctx.fcs ? '=== FLASHCARDS ===\n' + ctx.fcs.slice(0, 600) + '\n\n' : '') +
(ctx.qs ? '=== PAST QUESTIONS ===\n' + ctx.qs.slice(0, 600) + '\n\n' : '') +
'CONTENT BOUNDARY: Draw primarily from the content above. If asked about something
not covered, say so clearly and offer to help with what IS in the notes. Use general ' + board + '
...
+ board + ' GCSE ' + subjName + ' knowledge. Note when content is not in the student\'s
notes.';
...
'Reference ' + board + ' mark scheme language. Keep responses focused — avoid
padding.\n' +
imgInstr;
}
