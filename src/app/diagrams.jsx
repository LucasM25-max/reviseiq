...
export function KnowledgeGraph({
  ...
  const [etype, setEType] = React.useState("requires");
  React.useEffect(() => {
    try {
      setGraph(
        JSON.parse(
          localStorage.getItem(SK_GRAPH(user, subjectId)) ||
            '{"nodes":[],"edges":[]}',
        ),
      );
    } catch (_) {
  ...
  const nodes = (graph.nodes || [])
    ...
    .map((n) => ({
      ...n,
      mastery: Number(masteryMap?.[n.id] ?? n.mastery ?? 0),
    }));
  const frontier = calculateFrontier(
    { nodes, edges: graph.edges || [] },
    masteryMap,
  );
  const pos = React.useMemo(
    () => getNodePositions(nodes, 560, 360),
    [JSON.stringify(nodes)],
  );
  const save = (g) => {
    setGraph(g);
    try {
      localStorage.setItem(SK_GRAPH(user, subjectId), JSON.stringify(g));
    } catch (_) {}
  };
  return (
    <div style={{ ...C(D), padding: 14, marginTop: 12 }}>
      ...
      <svg viewBox="0 0 560 360" style={{ width: "100%", maxWidth: 560 }}>
        ...
        {(graph.edges || []).map((e, i) => {
          const a = pos.find((n) => n.id === e.from),
            b = pos.find((n) => n.id === e.to);
          if (!a || !b) return null;
          return (
            <line
              key={i}
              ...
              y2={b.y}
              stroke="#94a3b8"
              strokeWidth={Math.max(1, Number(e.weight) || 1)}
            />
          );
        })}
        {pos.map((n, i) => {
          const col =
            n.mastery > 70 ? "#16a34a" : n.mastery < 40 ? "#9ca3af" : "#f59e0b";
          const isFront = frontier.includes(n.id);
          return (
            <g
              key={i}
              onClick={() => {
                setSelected(n.id);
                onSelectNode && onSelectNode(n.id);
                const unmet = checkPrerequisites(graph, n.id, masteryMap, 60);
                if (unmet.length && onGoToPrereq) onGoToPrereq(unmet[0], n.id);
              }}
            >
              <circle
                cx={n.x}
              ...
              {Number(n.examFrequency || 0) > 7 && (
                <text x={n.x + 14} y={n.y - 12} fontSize="10">
                  {" "}
                </text>
              )}
            </g>
        ...
        <button
          onClick={() => {
            ...
            save({
              ...
              nodes: [
                ...(graph.nodes || []),
                { id: uid(), name: name.trim(), mastery: 0, examFrequency: 0 },
              ],
            });
            setName("");
          }}
      ...
      >
        ...
        <select
          ...
          style={{ ...I(D, { fontSize: 12 }) }}
        >
          {["requires", "contrasts", "explains", "example", "caused_by"].map(
            (t) => (
...
export function SketchnoteCanvas({ D, user, subjectId }) {
  ...
  React.useEffect(() => {
    try {
      setData(
        JSON.parse(localStorage.getItem(key) || '{"nodes":[],"edges":[]}'),
      );
    } catch (_) {
      setData({ nodes: [], edges: [] });
    }
    ...
    save({
      ...
      nodes: [
        ...data.nodes,
        {
          id: uid(),
          type,
          x: 80 + data.nodes.length * 18,
          y: 70 + data.nodes.length * 10,
          content: type,
        },
      ],
    });
  const onDown = (id, e) => {
    if (mode === "connect") {
      if (sel && sel !== id) {
        save({
          ...data,
          edges: [...data.edges, { from: sel, to: id, label: "" }],
        });
        setSel(null);
      } else setSel(id);
      return;
    ...
    save({
      ...
      nodes: data.nodes.map((n) =>
        n.id === drag.id
          ? {
              ...n,
              x: n.x + (e.clientX - drag.ox) / zoom,
              y: n.y + (e.clientY - drag.oy) / zoom,
            }
          : n,
      ),
    });
    setDrag({ id: drag.id, ox: e.clientX, oy: e.clientY });
  };
  const onUp = () => setDrag(null);
  return (
    <div
      style={{ ...C(D), padding: 12, marginTop: 12 }}
      onMouseMove={onMove}
      onMouseUp={onUp}
      onMouseLeave={onUp}
    >
      <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
        <button onClick={() => addNode("rect")}>+ Rect</button>
        <button onClick={() => addNode("circle")}>+ Circle</button>
        <button onClick={() => addNode("cloud")}>+ Cloud</button>
        <button
          onClick={() => setMode(mode === "connect" ? "move" : "connect")}
        >
          {mode === "connect" ? "Move" : "Connect"}
        </button>
        <button
          onClick={() =>
            sel &&
            save({
              ...data,
              nodes: data.nodes.filter((n) => n.id !== sel),
              edges: data.edges.filter((e) => e.from !== sel && e.to !== sel),
            })
          }
        >
          Delete
      ...
      >
        {data.edges.map((e, i) => {
          const a = data.nodes.find((n) => n.id === e.from),
            b = data.nodes.find((n) => n.id === e.to);
          if (!a || !b) return null;
          return (
            <g key={i}>
              <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="#94a3b8" />
              {e.label && (
        ...
        {data.nodes.map((n) => (
          ...
          >
            {n.type === "circle" ? (
              ...
              <path
                d={`M ${n.x - 26} ${n.y} C
${n.x - 32} ${n.y - 18}, ${n.x - 10} ${n.y - 30}, ${n.x + 8} ${n.y - 20} C ${n.x + 26} ${n.y - 28}, ${n.x + 36}
${n.y - 6}, ${n.x + 22} ${n.y + 8} C ${n.x + 8} ${n.y + 22}, ${n.x - 18} ${n.y + 20}, ${n.x - 26} ${n.y}`}
                fill="#e9d5ff"
                stroke={sel === n.id ? "#7c3aed" : "#6d28d9"}
              />
            ) : (
              <rect
                x={n.x - 26}
